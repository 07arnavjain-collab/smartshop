...
# Anchor to this file's own directory so relative paths ("data/...",
# "models/...") always resolve the same way, no matter what directory the
# process was launched from - this is also passed as cwd to the
# preprocess.py subprocess below so both processes agree.
...
<style>
    ...
    /* Cards for product results – applied to st.container(border=True) */
    div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlockBorderWrapper"] {
        background: rgba(18, 18, 37, 0.6) !important;
...
def system_initialized():
    return (
        ...
        os.path.exists("data/fake_reviews.csv") and
    ...
    if not has_creds:
        ...
        if st.button("Initialize System Now"):
            with st.spinner("Setting up system... This downloads ~60MB of data and trains models... Please wait."):
                # Run preprocessor
                try:
                    result = subprocess.run([sys.executable, "preprocess.py"], cwd=BASE_DIR, capture_output=True, text=True, check=True)
...
with tab1:
    ...
    if query:
        ...
        if results.empty:
            ...
            # Convert to list for 2-column grid iteration
            result_rows = list(results.iterrows())
            
            # Render products in a 2-column grid
            for row_start in range(0, len(result_rows), 2):
                grid_cols = st.columns(2)
                
                for col_idx in range(2):
                    item_index = row_start + col_idx
                    if item_index >= len(result_rows):
                        break
                    
                    idx, row = result_rows[item_index]
                    
                    with grid_cols[col_idx]:
                        # Each product is wrapped in a bordered container
                        # so the card AND its expander stay together inline
                        with st.container(border=True):
                            ...
                            # Product title
                            st.markdown(f"**{row['title'][:80]}{'...' if len(str(row['title'])) > 80 else ''}**")
                            st.markdown(
                            ...
                            price_display = f"₹{row['price_clean']:.2f}" if row['price_clean'] > 0 else "Price N/A"
                            mrp_display = f"~~₹{row['mrp_clean']:.2f}~~" if row['mrp_clean'] > 0 else ""
                            
                            # Price + Rating + Trust in compact layout
                            if mrp_display:
                                st.markdown(f"**{price_display}** {mrp_display} &nbsp;|&nbsp; ⭐ **{row['rating']:.1f}**", unsafe_allow_html=True)
                            else:
                                st.markdown(f"**{price_display}** &nbsp;|&nbsp; ⭐ **{row['rating']:.1f}**")
                            
                            auth_color = "green" if authenticity_rate >= 70 else "red" if authenticity_rate < 40 else "orange"
                            st.markdown(f"🛡️ Trust: <span style='color:{auth_color}; font-weight:bold;'>{authenticity_rate:.0f}%</span>", unsafe_allow_html=True)

                            # Inline expander — INSIDE the container so it
                            # expands directly below this card, not at the
                            # bottom of all products.
                            with st.expander("📄 View Details"):
                                p = row
                                p_matches = search_engine.df[search_engine.df['uniq_id'] == row['uniq_id']].index
                                p_idx = p_matches[0] if len(p_matches) > 0 else 0
                                ...
                                trust_percent = (1 - (fake_count / len(reviews))) * 100 if reviews else 100

                                st.progress(trust_percent / 100.0)
                                st.markdown(f"**{trust_percent:.1f}%** Genuine &nbsp;|&nbsp; **{100-trust_percent:.1f}%** Fake")

                                st.markdown("#### Customer Reviews Auditing")
                                if not labeled_revs:
                                    st.warning("⚠️ No reviews loaded for this product.")
                                for r in labeled_revs:
                                    ...
                                    st.markdown(
                                        ...
                                        f"  <p style='margin-top: 8px;'>\"{r['text'][:300]}\"</p>"
                                        f"</div>",
                                        unsafe_allow_html=True
                                    )
                                ...
                                for r_idx, rec_row in recs.iterrows():
                                    st.markdown(f"- **{rec_row['title'][:50]}...** — ₹{rec_row['price_clean']:.2f} | ⭐{rec_row['rating']:.1f}")

# ----------------- Tab 2: Review Authenticator -----------------
with tab2:
    st.subheader("🛡️ Real-Time Review Authenticity Checker")
    st.markdown("<span class='kaggle-badge'>📦 Model trained on real Kaggle dataset: <b>sudarshan24py/fake-reviews-dataset</b></span>", unsafe_allow_html=True)
